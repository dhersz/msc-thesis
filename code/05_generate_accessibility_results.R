...
parchin_generate_accessibility_results <- function(routes_info, fare_schema, n_cores, res = 7) {
  
  # routes_info <- generate_routes_info()
  # fare_schema <- generate_fare_schema()
  
  itineraries_details <- setDT(readr::read_rds(paste0("./data/rio_res_8/itineraries/itineraries_0700am.rds")))
  ...
  leg_cost_with_BU <- furrr::future_map(legs$legs, parchin_calculate_fare, fare_schema, BU = TRUE, .progress = TRUE)
  leg_cost_with_BU <- data.table::rbindlist(leg_cost_with_BU)
  data.table::setnames(leg_cost_with_BU, "leg_cost", "leg_cost_with_BU")
  
  leg_cost_without_BU <- furrr::future_map(legs$legs, parchin_calculate_fare, fare_schema, BU = FALSE, .progress = TRUE)
...
parchin_calculate_fare <- function(legs, fare_schema, BU = TRUE) {
